...
import sys
sys.stdout = open('teste.txt', 'w')


hoje = datetime.date.today()
#pega data de uma semana atras
url = 'http://devcolab.each.usp.br/do/catalog.json?f%5Bdata%5D%5B%5D=month_1&per_page=100&q=audiencia+publica'
...
def main():
    ...
    for audiencia in audiencias:
        #final = audiencias[audiencia]['texto'].find("((TEXTO))")
        #print (audiencias[audiencia]['secretaria'])
        #objt = '{ title:' + audiencias[audiencia]['secretaria'] + ', date: ' + datas[audiencia] + ', url:' + links[audiencia] + '}'
